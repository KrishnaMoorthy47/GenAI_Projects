...
from mealplanner.config import get_llm, get_settings

SYSTEM_PROMPT = """
You are an expert at designing nutritious meals that toddlers love.
You will be prompted to generate a weekly dinner plan.
You'll have access to meal preferences. Use these as inspiration but you don't have to explicitly use these items.
...
@tool
def get_kid_preferences() -> dict:
    """Get the likes and dislikes for the kids preferences."""
    client = MongoClient(os.getenv("MONGODB_URI", "mongodb://localhost:27017"))
    result = client["meal_planner"]["meal_preferences"].find_one({}, {"likes": 1, "dislikes": 1, "_id": 0})
    client.close()
    return result or {"likes": [], "dislikes": []}


@tool
def get_hard_requirements() -> list:
    """Get the hard requirements for recommending a meal. These must be enforced."""
    client = MongoClient(os.getenv("MONGODB_URI", "mongodb://localhost:27017"))
    result = client["meal_planner"]["meal_preferences"].find_one({}, {"hardRequirements": 1, "_id": 0})
    client.close()
...
async def generate_child_meal_plan() -> dict:
    llm = get_llm(temperature=0.7)
    agent = create_react_agent(llm, [get_kid_preferences, get_hard_requirements, get_recent_meals], prompt=SYSTEM_PROMPT)
    inputs = {"messages": [HumanMessage(content="Plan 7 dinners for my children.")]}
    ...
    try:
        ...
        pass
    return {"error": "Could not parse child meal plan", "raw": content}
